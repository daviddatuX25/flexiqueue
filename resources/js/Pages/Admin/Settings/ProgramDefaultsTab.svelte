<script lang="ts">
	...
	async function loadSettings() {
		...
		displayAudioVolume = Math.max(0, Math.min(1, Number((s as { display_audio_volume?: number }).display_audio_volume ?? 1)));
		displayTtsRepeatCount = Math.max(1, Math.min(3, Math.floor(Number((s as { display_tts_repeat_count?: number }).display_tts_repeat_count ?? 1))));
		displayTtsRepeatDelaySec = Math.max(0.5, Math.min(10, (Number((s as { display_tts_repeat_delay_ms?: number }).display_tts_repeat_delay_ms ?? 2000) / 1000)));
		allowPublicTriage = (s as { allow_public_triage?: boolean }).allow_public_triage === true;
		identityBindingMode = ((s as { identity_binding_mode?: string }).identity_binding_mode === "required" ? "required" : "disabled") as "disabled" | "required";
		allowUnverifiedEntry = (s as { allow_unverified_entry?: boolean }).allow_unverified_entry === true;
		enableDisplayHidBarcode = (s as { enable_display_hid_barcode?: boolean }).enable_display_hid_barcode !== false;
		enablePublicTriageHidBarcode = (s as { enable_public_triage_hid_barcode?: boolean }).enable_public_triage_hid_barcode !== false;
		enableDisplayCameraScanner = (s as { enable_display_camera_scanner?: boolean }).enable_display_camera_scanner !== false;
		enablePublicTriageCameraScanner = (s as { enable_public_triage_camera_scanner?: boolean }).enable_public_triage_camera_scanner !== false;
		const tts = (s as { tts?: { active_language?: string } }).tts;
		const lang = (tts?.active_language as string | undefined) ?? "en";
		ttsActiveLanguage = (["en", "fil", "ilo"].includes(lang) ? lang : "en") as "en" | "fil" | "ilo";
	}
	...
	async function handleSave() {
		...
		const { ok, message } = await api("PUT", apiBase, {
			settings: {
				...
				display_tts_repeat_delay_ms: Math.round(displayTtsRepeatDelaySec * 1000),
				...
				tts: { active_language: ttsActiveLanguage },
			},
		});
		submitting = false;
...
{:else}
	<div class="rounded-container bg-surface-50 border border-surface-200 shadow-sm p-6 space-y-6">
		...
		<div class="border-t border-surface-200 pt-6 mt-4 space-y-6">
			<div class="flex flex-col sm:flex-row gap-4">
				<div class="sm:w-1/3 shrink-0">
					<h3 class="font-medium text-surface-950 flex items-center gap-2"><Clock class="w-4 h-4 text-surface-500" /> Display auto-close</h3>
					<p class="text-xs text-surface-500 mt-1">Seconds before the Scan card auto-closes on Display and Status screens (0 = never).</p>
				</div>
				<div class="sm:w-2/3 flex items-center gap-2">
					<input type="number" min="0" max="300" class="input rounded-container border border-surface-200 px-3 py-2 w-24" bind:value={displayScanTimeoutSeconds} />
			...
			<div class="flex flex-col sm:flex-row gap-4">
				<div class="sm:w-1/3 shrink-0">
					...
					<p class="text-xs text-surface-500 mt-1">Default volume and repeat behavior when announcements play on the Display board.</p>
				</div>
				<div class="sm:w-2/3 space-y-3">
					...
					<div class="flex items-center gap-3">
						<label class="text-sm text-surface-700 w-32" for="cfg-default-volume">Volume</label>
						<input id="cfg-default-volume" type="range" min="0" max="1" step="0.05" class="range range-xs flex-1" bind:value={displayAudioVolume} />
						<span class="w-10 text-right text-xs text-surface-600">{Math.round(displayAudioVolume * 100)}%</span>
					</div>
					<div class="flex flex-wrap items-center gap-3">
						<label class="text-sm text-surface-700" for="cfg-default-repeat-count">Repeat</label>
						<select id="cfg-default-repeat-count" class="select rounded-container border border-surface-200 px-3 py-1 h-9" bind:value={displayTtsRepeatCount}>
							<option value={1}>Once</option>
							<option value={2}>Twice</option>
							<option value={3}>Three times</option>
						</select>
						<span class="text-xs text-surface-500">Delay between repeats</span>
						<input type="number" min="0.5" max="10" step="0.5" class="input w-20 px-2 py-1 text-right" bind:value={displayTtsRepeatDelaySec} />
						<span class="text-xs text-surface-600">seconds</span>
					</div>
					<div class="flex items-center gap-3">
						<label class="text-sm text-surface-700 w-32" for="cfg-default-tts-language">TTS language</label>
						<select id="cfg-default-tts-language" class="select rounded-container border border-surface-200 px-3 py-1 h-9 w-40" bind:value={ttsActiveLanguage}>
