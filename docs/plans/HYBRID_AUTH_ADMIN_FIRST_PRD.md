...
| **2026-03-22** | **H1 implemented in app:** `user_credentials` (local provider + password hash), `users.username` (unique), `users.recovery_gmail`, session login by **username** + `users.password` (kept in sync with local credential via `UserLocalCredentialService` from [`UserProvisioningService`](../../app/Services/UserProvisioningService.php) on relevant `User` saves). Admin create/update API + Staff UI require username + recovery Gmail. |
| **2026-03-22** | **H2 implemented in app:** Laravel Socialite; `GET /auth/google`, `GET /auth/google/callback`; `GoogleOAuthUserResolver` matches existing user by `user_credentials` (provider `google`) or `recovery_gmail`; no auto-provision; login page shows Google button when `GOOGLE_CLIENT_ID` is set. |
| **2026-03-22** | **H3 implemented in app:** Forgot password by **username**; reset email to **`recovery_gmail`** (Laravel `ResetPassword` notification + `routeNotificationForMail`); reset link uses **username** query (not `users.email`); Inertia pages `/forgot-password`, `/reset-password/{token}`; throttled routes. **Local:** Mailpit in `compose.yaml`; use `MAIL_HOST=mailpit` / port `1025` to catch mail. Production Google OAuth client IDs deferred until real domain on Agila. |
| **2026-03-22** | **H4 implemented in app:** Admin **POST** `/api/admin/users/{user}/reset-password` requires **`password` + `password_confirmation`**; **403** if admin targets self; **`AdminActionLog`** action `user_password_reset_by_admin` (target username only). Admin Users UI: fail-safe **temporary password** block for **other** users; self still uses optional password on **PUT** profile/update path. |
| **2026-03-22** | **H6 — docs:** [`docs/DEPLOYMENT.md`](../DEPLOYMENT.md) documents **Hestia/Agila SMTP**, **SPF/DKIM/DMARC**, and **no public registration** (admin-only onboarding). |
| **2026-03-22** | **H5 implemented in app:** `users.pending_assignment` (default false); **`EnforcePendingAssignment`** middleware — **staff** with flag may use `/pending-assignment`, `/profile`, `api/profile/*`, `api/users/me/*`, **POST** `/logout` only; **403** JSON on other `api/*`; **redirect** from staff routes. Login / guest redirect uses holding page when applicable. **Admin API:** `pending_assignment` on create (staff) + update; **assign station** clears flag; promote to non-staff clears flag. Admin Users UI: onboarding checkbox + **Pending** badge. |
| **2026-03-22** | **PROF-1 / AUTH-5 completion:** **`GET /auth/google/link`** (authenticated) + callback branch to attach Google when OAuth email matches **`recovery_gmail`**; **`DELETE /api/profile/google`** unlink; Profile UI card; **`EnforcePendingAssignment`** allows OAuth routes for pending staff. Ops runbook: [`docs/GOOGLE_OAUTH_AND_AGILA_HOSTING.md`](../GOOGLE_OAUTH_AND_AGILA_HOSTING.md). Shared helper **`App\Support\GoogleOAuthConfig`**. OAuth routes throttled; callback no longer uses `guest` middleware (supports link round-trip). |
