...
{
    /**
     * @see https://github.com/spatie/laravel-permission/blob/main/database/migrations/add_teams_fields.php.stub
     */
    public function up(): void
    {
        ...
        if (! Schema::hasColumn($tableNames['model_has_permissions'], $columnNames['team_foreign_key'])) {
            Schema::table($tableNames['model_has_permissions'], function (Blueprint $table) use ($tableNames, $columnNames, $pivotPermission) {
                $table->unsignedBigInteger($columnNames['team_foreign_key'])->default((string) RbacTeam::GLOBAL_TEAM_ID);
        ...
        DB::table($tableNames['roles'])->whereNull('team_id')->update(['team_id' => RbacTeam::GLOBAL_TEAM_ID]);
    ...
    {
        // Irreversible without data loss; Phase 6 rollback is manual per deploy runbook.
    }
};
