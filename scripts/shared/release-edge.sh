...
if [ -n "${1:-}" ]; then
    ...
    if [ -z "$VERSION" ]; then
        echo "Error: No version passed and no git tag found. Create a tag or pass version." >&2
        exit 1
    fi
fi
...
(cd "$REPO_ROOT" && $COMPOSE_CMD run --rm \
  -e WWWUSER -e WWWGROUP -e VERSION -e VITE_REVERB_APP_KEY -e VITE_REVERB_HOST -e VITE_REVERB_PORT -e VITE_REVERB_SCHEME -e VITE_REVERB_VIA_PROXY \
  ...
  tar -czf flexiqueue-${VERSION}-edge.tar.gz \
    --exclude=.git --exclude=node_modules --exclude=.env --exclude=storage --exclude=tests --exclude=e2e --exclude=.github --exclude=php-run-scripts .
')

TARBALL="$REPO_ROOT/flexiqueue-${VERSION}-edge.tar.gz"
[ -f "$TARBALL" ] || { echo "Error: Tarball was not created: $TARBALL" >&2; exit 1; }
